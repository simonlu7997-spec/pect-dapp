...
export default function Staking() {
  ...
  const [txType, setTxType] = useState<"stake" | "unstake" | "claim" | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);

  const { data: stakingInfo, isLoading, refetch } = trpc.staking.getStakingInfo.useQuery(
    { walletAddress },
    { enabled: !!walletAddress, refetchInterval: 30_000 }
  );

  const { data: history, refetch: refetchHistory } = trpc.staking.getStakingHistory.useQuery(
    { walletAddress },
    { enabled: !!walletAddress }
  );

  const recordTxMutation = trpc.staking.recordStakingTx.useMutation();
  ...
  const handleStake = async () => {
    ...
    if (amount > available) {
      toast.error(`余额不足，当前可用 ${formatAmount(stakingInfo?.c2Balance, 2)} C2C`);
      return;
    }
    ...
    try {
      ...
      await recordTxMutation.mutateAsync({
        walletAddress, txHash: tx.hash, txType: "stake", amount: stakeAmount, tokenSymbol: "C2C",
      });
      await tx.wait(1);
  ...
  const handleUnstake = async () => {
    ...
    if (amount > staked) {
      toast.error(`超出已质押数量，当前已质押 ${formatAmount(stakingInfo?.stakedAmount, 2)} C2C`);
      return;
    }
    setIsProcessing(true);
    setTxType("unstake");
    try {
      ...
      setTxHash(tx.hash);
      await recordTxMutation.mutateAsync({
        walletAddress, txHash: tx.hash, txType: "unstake", amount: unstakeAmount, tokenSymbol: "C2C",
      });
      await tx.wait(1);
  ...
  const handleClaimReward = async () => {
    if (!signer || !STAKING_MANAGER_ADDRESS) { toast.error("合约地址未配置"); return; }
    if (!stakingInfo?.pendingReward || parseFloat(stakingInfo.pendingReward) <= 0) {
      toast.error("暂无可领取的奖励");
      return;
    }
    setIsProcessing(true);
    setTxType("claim");
    try {
      ...
      setTxHash(tx.hash);
      await recordTxMutation.mutateAsync({
        walletAddress, txHash: tx.hash, txType: "claim_reward",
        amount: stakingInfo.pendingReward, tokenSymbol: "USDT",
      });
      await tx.wait(1);
      ...
      <div className="container max-w-4xl py-10 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">质押 C2-Coin 获取 USDT 奖励，支持碳中和目标</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="border-emerald-200">
            <CardHeader className="pb-2">
              <CardDescription className="flex items-center gap-1 text-xs">
                <Coins className="w-3.5 h-3.5" /> C2C 余额
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-8 w-24" /> : (
                <><p className="text-2xl font-bold text-gray-900">{formatAmount(stakingInfo?.c2Balance, 2)}</p><p className="text-xs text-gray-400">C2C</p></>
              )}
            </CardContent>
          </Card>

          <Card className="border-emerald-200 bg-gradient-to-br from-blue-50 to-indigo-50">
            <CardHeader className="pb-2">
              <CardDescription className="flex items-center gap-1 text-xs">
                <BarChart3 className="w-3.5 h-3.5" /> 已质押
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-8 w-24" /> : (
                <><p className="text-2xl font-bold text-blue-700">{formatAmount(stakingInfo?.stakedAmount, 2)}</p><p className="text-xs text-gray-400">C2C</p></>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50">
            <CardHeader className="pb-2">
              <CardDescription className="flex items-center gap-1 text-xs">
                <TrendingUp className="w-3.5 h-3.5" /> 待领取奖励
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-8 w-24" /> : (
                <><p className="text-2xl font-bold text-emerald-700">{formatAmount(stakingInfo?.pendingReward, 2)}</p><p className="text-xs text-gray-400">USDT</p></>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-emerald-200">
            <CardHeader className="pb-2">
              <CardDescription className="flex items-center gap-1 text-xs">
                <TrendingUp className="w-3.5 h-3.5" /> 年化收益率
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-8 w-24" /> : (
                <><p className="text-2xl font-bold text-gray-900">{stakingInfo?.apy || "12"}%</p><p className="text-xs text-gray-400">APY</p></>
              )}
            </CardContent>
          </Card>
        </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-3xl font-bold text-emerald-700 mt-1">
                    {formatAmount(stakingInfo.pendingReward, 2)} USDT
                  </p>
                </div>
                <Button
        ...
        <Tabs defaultValue="stake">
          <TabsList className="grid grid-cols-2 w-full max-w-xs">
            <TabsTrigger value="stake" className="gap-1.5">
              <ArrowUpRight className="w-4 h-4" />质押
            </TabsTrigger>
            <TabsTrigger value="unstake" className="gap-1.5">
              <ArrowDownLeft className="w-4 h-4" />解除质押
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="stake">
            <Card className="border-emerald-200">
              <CardHeader>
                ...
                <CardDescription>
                  质押您的 C2-Coin 以获取 USDT 奖励（年化 {stakingInfo?.apy || "12"}%）
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setStakeAmount(stakingInfo?.c2Balance || "0")}
                  ...
                  <p className="text-xs text-gray-400">
                    可用余额：{formatAmount(stakingInfo?.c2Balance, 2)} C2C
                  </p>
                </div>
                ...
                {stakeAmount && parseFloat(stakeAmount) > 0 && (
                  <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 space-y-1 text-sm">
                    ...
                    <p className="text-blue-600">
                      年化收益：约{" "}
                      {(parseFloat(stakeAmount) * parseFloat(stakingInfo?.apy || "12") / 100)
                ...
                <Button
                  ...
                  disabled={
                    ...
                    !stakingInfo?.contractConfigured
                  }
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 text-lg font-semibold"
                  size="lg"
                >
                  {isProcessing && txType === "stake" ? (
            ...
            <Card className="border-emerald-200">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-400">
                    已质押：{formatAmount(stakingInfo?.stakedAmount, 2)} C2C
                  </p>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">
                          ...
                          <span
                            className={`font-bold ${
                              tx.txType === "unstake" ? "text-red-600" : "text-emerald-600"
                            }`}
                          >
                            {tx.txType === "unstake" ? "-" : "+"}
                            {formatAmount(tx.amount, 2)} {tx.tokenSymbol}
                          </span>
                        </p>
                        <p className="text-xs text-gray-400">
