...
// PrivateSale 合约 ABI（只需要查询和购买相关函数）
...
function getContracts() {
  ...
  return { rpcUrl, privateSaleAddress, usdtAddress };
}
...
export const purchaseRouter = router({
  ...
  getPrivateSaleInfo: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      // 合约未配置时返回默认值（前端展示占位数据）
      if (!privateSaleAddress || privateSaleAddress === "0x0000000000000000000000000000000000000000") {
        return {
          contractConfigured: false,
          isActive: false,
          tokenPrice: "0.10",       // USDT per PVC
          totalRaised: "0",
          hardCap: "80000",
          minPurchase: "500",       // USDT
          maxPurchase: "10000",     // USDT
          progressPercent: 0,
          userPurchased: "0",       // USDT spent by this wallet
          userUsdtBalance: "0",
          userAllowance: "0",
        };
      }
      ...
      try {
        ...
        const privateSale = new ethers.Contract(privateSaleAddress, PrivateSaleABI, provider);

        const [tokenPrice, totalRaised, hardCap, minPurchase, maxPurchase, isActive] =
          await Promise.all([
            privateSale.tokenPrice(),
            privateSale.totalRaised(),
            privateSale.hardCap(),
            privateSale.minPurchase(),
            privateSale.maxPurchase(),
            privateSale.isActive(),
          ]);
        ...
        if (usdtAddress && input.walletAddress) {
          const usdt = new ethers.Contract(usdtAddress, ERC20ABI, provider);
          const [decimals, balance, allowance, purchased] = await Promise.all([
            usdt.decimals(),
            usdt.balanceOf(input.walletAddress),
            usdt.allowance(input.walletAddress, privateSaleAddress),
            privateSale.purchasedAmount(input.walletAddress),
          ]);
        ...
        const progressPercent = Number(hardCap) > 0
          ? Math.min(100, Math.round((Number(totalRaised) / Number(hardCap)) * 100))
          : 0;

        // tokenPrice 是 USDT per PVC（6 位精度）
        const tokenPriceFormatted = ethers.formatUnits(tokenPrice, usdtDecimals);
  ...
  recordPurchase: publicProcedure
    .input(
      z.object({
        ...
        usdtAmount: z.string(),   // USDT 金额（字符串，避免精度丢失）
        pvcAmount: z.string(),    // PVC 数量
