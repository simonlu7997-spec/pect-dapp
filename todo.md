...
- [x] 后端：oracle 新增 getStationBreakdown 接口（各电站发电量/收入对比）
- [x] 前端：Analytics 页面接入真实 oracle 数据（替换所有 Mock 数据）
- [x] 前端：新增柱状图（月度发电量 vs 收入双 Y 轴对比）
...
- [x] 新建 Calculator.tsx 页面（私募/公募切换 + 投入金额输入 + 收益预测展示）
- [x] 后端：oracle.getStats 接口已有，复用现有数据（年化收益率、分红比例等）
- [x] 前端：计算逻辑（PVC 数量、预期月/年分红、回本周期）
- [x] 前端：收益对比图表（私募 vs 公募 收益曲线）
- [x] 在导航栏和首页 CTA 区域添加入口

## 2026-05-18 收益计算器入口 + PVC 总量修正

- [x] 首页 Hero 区域添加“估算收益”按钮（跳转 /calculator）
- [x] 购买页面金额输入框下方添加“查看预期收益 →”链接
- [x] 全站 PVC 总量从 1000 万修正为 400 万枚（Calculator.tsx 已修正，其他页面无硬编文字）

## 2026-05-18 导航栏移动端优化

- [x] 将“数据分析”和“收益计算”合并到“工具”下拉菜单

## 2026-05-18 计算器 URL 参数自动填入

- [x] Calculator.tsx 读取 ?amount=xxx&type=private/public，自动填入金额并切换对应 Tab

## 2026-05-18 按白皮书 V6.1 更新计算器公式

- [x] 按白皮书 V6.1 §5.3.2 重写计算逻辑：前24月 75% 代币参与分红，24月后 100% 代币参与分红
- [x] 私募年化收益率 17.15%（前24月）/ 12.86%（24月后）
...
## 2026-05-18 收益对比参考表动态化

- [x] 收益对比参考表四个年化数字改为从动态参数实时计算（dynamicMonthlyP1/P2 × 12 ÷ 代币价格），不再硬编码

## 2026-05-19 收益计算器加入 C2C 质押收益 + 汇率调整

- [x] 白皮书 V6.1 §5.4.1：加入 C2-Coin 质押奖励池（4,573 USDT/年）计算
- [x] 假设全网 C2C 质押率 50%，全网质押池总量 = 382,990 × 50% = 191,495 枚
- [x] 用户质押 C2C = PVC 数量 × 50%，质押收益 = 奖励池 × (用户质押 / 全网质押池)
- [x] CalcResult 新增综合收益字段：monthlyTotalP1/P2、annualTotalP1/P2、annualTotalYieldP1/P2
- [x] ResultCard 展示综合收益（PVC 分红 + C2C 质押），并在明细行显示各项拆分
- [x] 对比视图摘要卡片改为显示综合年化收益率
- [x] 收益对比参考表改为显示含 C2C 质押的综合年化收益率（不再硬编码）
- [x] AdminCalculatorParams.tsx 新增质押奖励池、C2C 年发行量、质押率三个配置字段
- [x] 管理后台实时预览改为显示综合年化收益率（含 C2C 质押）
- [x] 汇率默认值从 7.2 调整为 6.8，数据库已更新

## 2026-05-19 修正 C2C 质押收益计算公式

- [x] 修正 C2C 空投量计算：从错误的"PVC数量 × 50%"改为正确的"年发行量 × PVC持仓比例"（白皮书 §3.2.3 + §5.5 案例验证）
- [x] 正确公式：c2cAirdrop = 382,990 × (pvcAmount / 4,000,000)，质押 = c2cAirdrop × 50%
- [x] CalcResult 新增 c2cAirdropAmount 字段，ResultCard 明细行显示空投量和质押量
- [x] 验证：投资 10,000 USDT → 100,000 PVC → C2C空投 9,574.75 枚 → 质押 4,787.38 枚 → 年度质押奖励 ≈ 114 USDT

## 2026-05-19 三项改进

- [x] 收益对比参考表验证并修正为白皮书 §5.6 正确数字（公募14.86%/11.43%，私募18.58%/14.29%）
- [x] Buy.tsx 私募/公募年化标签接入动态计算（含 C2C 质押），与计算器保持一致
- [x] Calculator.tsx 底部参数说明改为动态显示当前配置値（汇率、质押率、C2C年发行量等）
